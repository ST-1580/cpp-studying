...
big_integer::big_integer() : value(), sign(false) {
    value.push_back(0);
}
...
big_integer::big_integer(int a) : value(), sign(a < 0) {
    if (a == std::numeric_limits<int>::min()) {
        value.push_back(2147483648);
    } else {
        value.push_back((a > 0) ? a : -a);
    }
}

big_integer::big_integer(uint32_t a) : value(), sign(false){
    value.push_back(a);
}

big_integer::big_integer(std::string const& str) : value(), sign(false) {
